...
class UserController extends AbstractController
{
     ...
     * @throws NotLoggedInException
     */
    public function listTopicsAction(?FrontendUser $user = null, int $page = 1): ResponseInterface
    {
     ...
     */
    public function listSubscriptionsAction(int $forumPage = 1, int $topicPage = 1): ResponseInterface
    {
        $user = $this->getCurrentUser();
        if ($user->isAnonymous()) {
     ...
     */
    protected function redirectToSubscriptionObject(SubscribeableInterface $object): ResponseInterface|null
    {
        if ($object instanceof Forum) {
            $uri = $this->uriBuilder->uriFor('show', ['forum' => $object], 'Forum');
            return $this->responseFactory->createResponse(307)
                ->withHeader('Location', $uri);
        }
        if ($object instanceof Topic) {
            $uri = $this->uriBuilder->uriFor('show', ['topic' => $object], 'Topic');
            return $this->responseFactory->createResponse(307)
                ->withHeader('Location', $uri);
        }
        return null;
