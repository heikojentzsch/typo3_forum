...
class GeSHi {
    ...
    function enable_strict_mode($mode = true) {
        if (array_key_exists('STRICT_MODE_APPLIES', $this->language_data)){
    ...
    function build_parse_cache() {
        ...
        if (array_key_exists('KEYWORDS', $this->language_data)){
...
//        $result = substr($result, 1, -1);
    ...
    function parse_non_string_part($stuff_to_parse) {
        ...
        if (array_key_exists('OOLANG', $this->language_data)){
    ...
    function _genCSSName($name){
        return (is_numeric($name[0]??false) ? '_' : '') . $name;
    }
